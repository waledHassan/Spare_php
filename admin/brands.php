...
if($action == 'manage'){
     @$id = $_GET['id'];
     $cats = $conn -> prepare("SELECT * FROM brands WHERE child_category = ?");
                  ...
                  <div class="card-image">
                    <a href="brands.php?id=<?php echo $row['id']?>">
                      <img style='height: 300px;'
                        src="assets/images/brands/<?php echo $row['image']?>"
                        alt=""
                      />
                    </a>
                  </div>
                ...
                <div class="row">
                  <div class="col-12">
                    <div class="input-style-1">
                      <label> Name</label>
                      <input name='name' type="text" placeholder="Sub Category Name"/>
                    </div>
                  </div>
                  </div>
                  <div class="col-12">
                    <div class="input-style-1">
                     ...
                     <select name="cat" class="form-control">
                        ...
                        <?php
                        $categories = getallFrom('*' ,'categories' , "", "", 'category_id');
                  ...
                  <div class="col-12">
                    <div class="input-style-1">
                     ...
                     <select name="subcat" class="form-control">
                        <option value="0">...</option>
                        <?php
                        $categories = getallFrom('*' ,'child_category' , "", "", 'id');
                        foreach($categories as $cat){
...
// #############################################################################
              ...
              if($_SERVER['REQUEST_METHOD'] == 'POST'){
                    ...
                    if(empty($cat)){
                      $formErrors[] = "Sorry Parent Category Name Can’t Be Empty";
                    }
                    if(empty($subcat)){
                      $formErrors[] = "Sorry Sub Category Name Can’t Be Empty";
                      }


                  foreach($formErrors as $error){
                        echo "<div class='alert alert-danger container text-center'>" .$error. "</div>";
                        header("refresh:2;url=brands.php?action=add");
          ...
          if(empty($formErrors)){
                ...
                move_uploaded_file($avatartmp , "assets\images\brands\\" . $avatar);
              ...
              if($count == 1){
                  ...
                  echo "<div class='alert alert-danger text-center container'>This Brand Name Is Exist</div>";
                  header("refresh:2;url=brands.php?action=add");
                  exit();

       }else{

    $stmt = $conn -> prepare("INSERT INTO brands
                      (name, category , child_category , add_date , image)
              VALUES
                      (:name , :id , :subcat , now() , :image)");
                 ...
                 <div class="title mb-30">
                   ...
                   <div class="row">
                     <div class="col-12">
                       <div class="input-style-1">
                         <label> Name</label>
                         <input name='name' type="text" placeholder="Sub Category Name" value='<?php echo $row['name'] ?>'/>
                       </div>
                     </div>
                     </div>
                     <div class="col-12">
                         ...
                         <select name="cat" class="form-control">
                            ...
                            foreach($categories as $cat){
                                    echo "<option value='".$cat['category_id']."'";
                                      if($row['id'] == $cat['category_id']){echo "selected";}
                         ...
                         <select name="subcat" class="form-control">
                            ...
                            foreach($categories as $cat){
                                    echo "<option value='".$cat['id']."'";
                                      if($row['id'] == $cat['id']){echo "selected";}
                                    echo ">".$cat['name']."</option>";
                            }
                            ?>
                        </select>
                     ...
                     <div class="col-12">
                       <div class="input-style-1">
                         <label>image</label>
                         <img src="assets/images/brands/<?php echo $row['image'] ?>" style='width:100px;height: 100px;margin: 10px 0 10px 200px;'/>
...
// #########################################################################################  
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST'){
                  ...
                  @$avatarExtention = strtolower( end ( explode ('.' , $avatarname) ) );
                    ...
                    $formErrors = [];
                    if(empty($name)){
                    $formErrors[] = "Sorry Brand Name Can’t Be Empty";
                    }
                    if(empty($cat)){
                    $formErrors[] = "Sorry Category Can’t Be Empty ";
                    }
                    if(empty($subcat)){
                      $formErrors[] = "Sorry Sub Category Can’t Be Empty ";
                      }


                foreach($formErrors as $error){
                      echo "<div class='alert alert-danger container text-center'>" .$error. "</div>";
                      header("refresh:2;url=brands.php?action=edit");
                   ...
                   $stmt = $conn -> prepare("SELECT * FROM brands WHERE name= ? AND id != ?");
                    $stmt ->execute(array($name , $id));
                    $count = $stmt -> rowCount();

                  if($count == 1){

                  echo "<div class='alert alert-danger text-center container'>This Sub Category Name Is Exist</div>";
                  header("refresh:2;url=brands.php?action=edit");
                  exit();

              }else{

                  $stmt = $conn -> prepare("UPDATE brands SET name= ?,category= ? , child_category = ?,image= ? WHERE id = ?");

                  $stmt -> execute(array($name , $cat , $subcat , $avatar , $id));
   ...
   // #####################################################################################   
        ...
        $stmt_delete = $conn -> prepare("SELECT * FROM brands WHERE id = ?");
